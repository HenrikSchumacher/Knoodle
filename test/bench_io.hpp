...
inline DataSet Load(const std::string& path)
{
    ...
    while (std::getline(in, line))
    {
        ...
        if (!have_count)
        {
            std::string tok;
            ss >> tok;
            if (tok != "count" || !(ss >> expected))
            {
                throw std::runtime_error("bench_io::Load: expected 'count N' before diagrams in " + path);
            }
            have_count = true;
        ...
        if (!(ss >> d.nc) || d.nc <= 0)
        {
            throw std::runtime_error("bench_io::Load: bad crossing count in " + path);
        }
    ...
    if (have_count && ds.diagrams.size() != expected)
    {
