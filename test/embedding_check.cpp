...
{
    return f.expect.XFail(tier,cls);
}
...
{
    const std::string reason = f.expect.XCrash(tier,cls);
                           ...
                           Tally & t, bool verbose )
{
    ...
    const std::string xfail = XFailFor(f,"exact",cls);
...
static void TierCross( const std::vector<Fixture> & fx, Coords coords,
                       Int rotations, Tally & t, bool verbose )
{
    ...
    for( const auto & f : fx )
    {
        ...
        auto compare = [&]( const kt::Curve & c, const std::string & tag, bool generic )
        {
            ...
            std::string xfail = XFailFor(f,"cross",2);
            if( xfail.empty() ) { xfail = XFailFor(f,"cross",3); }
            if( xfail.empty() ) { xfail = XFailFor(f,"cross",4); }
                               ...
                               Tally & t, bool verbose )
{
    ...
    const std::string xfail = XFailFor(f,"invariant",cls);
                ...
                for( Coords c : coords )
                {
                    for( int cls : classes )
                    {
                        ...
                        if( r.crashed )
                        {
                            ...
                            const std::string xc = f.expect.XCrash(tier.c_str(),cls);
