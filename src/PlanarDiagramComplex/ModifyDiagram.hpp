...
bool ConnectWithIsthmus(
    const Int diagram_idx, const Int a, const Int b, const CrossingState_T h
)
{
    auto tag = []()-> std::string { return MethodName("ConnectWithIsthmus"); };
    TOOLS_PTIMER(timer,tag());
    ...
    bool changedQ = pd.template ConnectWithIsthmus<silentQ,assertsQ>(a,b,h);
    if( changedQ ) { ClearCache(); }
    return changedQ;
}
...
bool ConnectWithDoubleIsthmus(
    ...
    const CrossingState_T h_0, const CrossingState_T h_1
)
{
    auto tag = []()-> std::string { return MethodName("ConnectWithDoubleIsthmus"); };
    TOOLS_PTIMER(timer,tag());
    ...
    bool changedQ = pd.template ConnectWithDoubleIsthmus<silentQ,assertsQ>(a,b,h_0,h_1);
    if( changedQ ) { ClearCache(); }
    return changedQ;
}
