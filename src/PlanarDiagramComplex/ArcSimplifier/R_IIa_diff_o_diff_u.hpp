bool R_IIa_diff_o_diff_u()
{
    ...
    PD_PRINT(MethodName("R_IIa_diff_o_diff_u")+": Delete 2 crossings. ( crossing_count = " + ToString(pd.crossing_count) + ")");

    // We cannot use Reconnect here because we have to change directions of vertical strands.
