...
/*!@brief Collapse the strand made by the arcs `a_begin, NextArc(a_begin,Head),...,a_end` to a single arc. We reconnect the head of arc `a_begin` to the head of `a_end` and deactivate `a_end`. Finally, we deactivate all the crossings from the head of `a_begin` up to the tail of `a_end`. This routine must only be called if some preparations have be carried out:
 *
 *      !!! all arcs that cross this strand have to be reconnected or deactivated first.!!!
 *
