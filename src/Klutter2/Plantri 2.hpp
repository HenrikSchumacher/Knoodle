...
void LoadPlantriPDCodes(ExtInt embedding_trials_, ExtInt rotation_trials_)
{
    ...
    RequireClassifiers();
    mref<Classifier> classifier = classifiers[Size_T(0)];
...
//        tic("Reading inputs");
    while( !s.EmptyQ() && !s.FailedQ() && !failedQ )
    {
        s.TakeMatrixFunction(
            pd_code.WriteAccess(),crossing_count,Int(4), "", "\n", "\n", "", " ", ""
        );
        
        if( s.FailedQ() )
        {
            eprint(tag() + ": Reading pd code no. " + ToString(input_count) + " from file \"" + pd_code_file.c_str() + "\" failed. Current position = " + ToString(s.Position()) + "; current character code = " + ToString(static_cast<int>(s.CurrentChar())) + "."
            );
            return;
        }
        ...
        PD_T pd_0 = PD_T::template FromPDCode<{.signQ = false,.colorQ = false}>(
            pd_code.data(), crossing_count, false, false
        );
...
//            wprint(tag() + ": Found diagram with more than one link component.");
        ...
        for( UInt64 i = 0; i < i_max; ++i )
        {
            ...
            {
                if( subklutters[I_S[invariant]].InsertUnidentifiedKey(ToKey(pd)) )
                {
                    eprint(tag() + ": InsertUnidentifiedKey failed.");
                    failedQ = true;
                    return;
                }
            }
            ...
            {
                // If this invariant has not appeared in the data from KnotInfo, then we can safely ignore this diagram. We do nothing.
            }
...
//    );
//    
//    // This "coarse sieving" jut culls all diagram that are not pass reduced. We dont apply symmetry operations, yet.
//    
//    tic("Coarse sieving");
//    ParallelDo(
...
//                    }
//                    
//                    // TODO: Compute the classifier of pd and store it in survivors.
//                    
//                    survivors.insert(ToKey(pd));
//                }
//                
...
//    );
//    
//     TODO: Sort survivors into the bucket_lists according to how they were classified.
//     TODO: Then loop over 
//    
...
//        
//    // Now we apply symmetry operatations and use Reapr to generate new diagrams.
//    
//    tic("Fine sieving");
...
//    }
    ...
    print(tag() + ": inputs read = " + ToString(input_count) + "; unidentified keys left (without symmetries) = " + ToString(UnidentifiedKeyCount()));
