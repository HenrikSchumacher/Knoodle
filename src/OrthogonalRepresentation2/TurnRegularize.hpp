    ...
    // Compute rotations.
    {
        ...
        do
        {
            if( dE_turn[de] == Turn_T(-1) )
            {
                ...
                {
                    eprint(ClassName()+"::FindKittyCorner: TREdge " + ToString(de) + " has bounday flag set to " + ToString(DedgeExteriorQ(de)) + ", but face's boundary flag is " + ToString(exteriorQ) + "." );
                }
            }
            ++dE_counter;
            rot += dE_turn[de];
...
//        TOOLS_DUMP(V_dTRE(c_1,s_1));
    ...
    if( exteriorQ )
    {
        const Int edge_count_0 = FaceEdgeCount(de_0);
        const Int edge_count_1 = FaceEdgeCount(de_1);
