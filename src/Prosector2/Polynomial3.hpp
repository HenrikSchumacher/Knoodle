...
//    }
    ...
    Sign_T Sign() const
    {
        if( c_0 < 0 ) return Sign_T(-1);
//        if( c_0.isNegative() ) return Sign_T(-1);
        if( c_0 > 0 ) return Sign_T( 1);
        
        if( c_1 < 0 ) return Sign_T(-1);
//        if( c_1.isNegative() ) return Sign_T(-1);
        if( c_1 > 0 ) return Sign_T( 1);
        
        if( c_3 < 0 ) return Sign_T(-1);
//        if( c_3.isNegative() ) return Sign_T(-1);
        if( c_3 > 0 ) return Sign_T( 1);
