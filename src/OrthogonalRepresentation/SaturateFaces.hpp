...
EdgeContainer_T SaturatingEdges() const
{
    ...
    Tensor1<Int,Int>      f_V ( max_face_size * Int(2) );
    ...
    TraverseAllFaces(
        ...
        [&exteriorQ,&f_size,&f_n,&f_V,&f_S,this](
        ...
        [&exteriorQ,&f_size,&f_n,&f_V,&f_S,&edge_agg,this]( const Int f )
        {
            ...
            if( f_n < Int(2) )
            {
                eprint(ClassName() + "::SaturatingEdges: f_n < Int(2)!");
            }
            
            // DEBUGGING
            if( f_size > max_face_size )
            {
                eprint(ClassName() + "::SaturatingEdges: f_size > max_face_size !");
            }
            
            // Make a complete copy to emulate a cyclic list.
            copy_buffer(&f_V[0],&f_V[f_n],f_n);
            copy_buffer(&f_S[0],&f_S[f_n],f_n);

            this->template SaturateFace<GrQ,verboseQ>(f,f_V,f_S,f_n,edge_agg);
        }
    );
...
) const
{
    ...
    if constexpr ( verboseQ )
    {
        ...
        print( ArrayToString(f_V.data(), {f_n*Int(2)} ) );
    ...
    auto update_buffer = [&f_n,&f_V,&f_S]( const Int i_2 )
    {
        ...
        // First idea was this:
        //      copy_buffer( &f_V[i_2], &f_V[i_0], f_n - i_0 );
        //      copy_buffer( &f_S[i_2], &f_S[i_0], f_n - i_0 );
        // (and then duplicated the beginning after the end).
        
        // Problem: We can have f_V[0] == f_V[i_1], i.e., we have to remove positions [1,...,i_0[ one step to the left and copy [i_2,...,i_2 + f_n - i_0[ to position i_1 instead. Or something like this.
        // This is super error-prone, so I go for a full duplicate of the face:
        
        // Face has positions [0,1,2,...,i_0,i_1,i_2,...2 * f_n[.
        
        // We have f_V[0] != f_V[i_0], so i_0 <= f_n - 1.
        // Thus, i_2 <= f_n + 1.
        ...
        std::copy( &f_V[i_2], &f_V[i_2+f_n], &f_V[0] );
...
template<bool GrQ, bool verboseQ = false>
EdgeContainer_T SaturatingEdges2() const
{
    TOOLS_PTIMER(timer,ClassName()+"::SaturatingEdges2<" + ToString(GrQ) + ">");
    ...
    TraverseAllFaces(
        ...
        [&exteriorQ,&f_size,&f_n,&f_dE,&f_S,&edge_agg,this]( const Int f )
        {
            ...
            this->template SaturateFace2<GrQ,verboseQ>(f,f_dE,f_S,f_n,edge_agg);
        }
    );
