...
Sparse::MatrixCSR<S,I,J> Lengths_ConstraintMatrix()
{
    ...
    for( Int e = 0; e < TRE_count; ++e )
    {
        ...
        if( dir == East )
        {
            const I i = static_cast<I>(Vs_offset + TRE_DvE[e]);
            TOOLS_DUMP(i);
        ...
        {
            const I i = static_cast<I>(Hs_offset + TRE_DhE[e]);
            TOOLS_DUMP(i);
...
{
    TOOLS_DUMP(Dh.EdgeCount());
    TOOLS_DUMP(DhE_costs.Size());
    
    TOOLS_DUMP(Dv.EdgeCount());
    TOOLS_DUMP(DvE_costs.Size());
    ...
    DhE_costs.Write(c.data() + Hs_offset);
    DvE_costs.Write(c.data() + Vs_offset);
