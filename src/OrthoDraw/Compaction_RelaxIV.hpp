...
void ComputeVertexCoordinates_Compaction_RelaxIV()
{
    TOOLS_PTIMER(timer,MethodName("ComputeVertexCoordinates_Compaction_RelaxIV"));
    ...
    ParallelDo(
        [&x,&y,this](const Int thread)
        {
            if( thread == Int(0) )
            {
                x = Compaction_RelaxIV( Dv(), DvEdgeCosts() );
            }
            else if( thread == Int(1) )
            {
                y = Compaction_RelaxIV( Dh(), DhEdgeCosts() );
            }
        },
        Int(2),
...
Tensor1<Int,Int> Compaction_RelaxIV( cref<DiGraph_T> D, cref<Tensor1<Cost_T,Int>> edge_costs )
{
    ...
    TOOLS_PTIMER(timer,MethodName("Compaction_RelaxIV"));

    {
        ...
        {
            eprint(MethodName("Compaction_RelaxIV") + ": Too many arcs to fit into type " + TypeName<I> + ".");
    ...
    Tensor1<R,I> capacities(m,Scalar::Max<R>);    // RelaxV does not like infty capacity on edges with negative costs.
    Tensor1<I,I> tails     (m);
    Tensor1<I,I> heads     (m);
    Tensor1<R,I> costs     (m);
