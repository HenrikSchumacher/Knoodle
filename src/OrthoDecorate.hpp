...
namespace Knoodle
{
    ...
    public:
        ...
        //
        // MOVED. The surgery is `PassDescriptor::AfterDiagram` now: it reads a
        // diagram and a descriptor and returns a diagram, so it belongs beside
        // the descriptor's own checks rather than in the drawing code, and a
        // verifier that draws nothing must not have to lay out a drawing to
        // call it. These forward, so `deco.AfterDiagram(pd,mv,...)` and
        // `Deco_T::AfterDiagram(pd,mv,...)` keep working.
        //======================================================================
        ...
        {
            return mv.AfterDiagram(pd,why,freed);
        }
