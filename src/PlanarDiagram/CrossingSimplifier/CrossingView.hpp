...
public:
    ...
    CrossingView( const Int c_, mptr<Int> C_, CrossingState_T & S_ )
    : c { c_ }
    , C { C_ }
    , S { S_ }
    {}
    ...
    CrossingState_T & State()
    {
        return S;
    }
    
    const CrossingState_T & State() const
    {
        return S;
    }
    ...
    bool ActiveQ() const
    {
        return (ToUnderlying(S) & Underlying_T<CrossingState_T>(1));
    }
...
void Deactivate( mref<CrossingView> C )
{
    ...
    {
        --pd.crossing_count;
        C.State() = CrossingState_T::Inactive;
    }
    else
    {
#if defined(PD_DEBUG)
