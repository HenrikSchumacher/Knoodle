    ...
    )
    {
        ...
        tic("Computing f");
        {
            for( Size_T rep = 0; rep < reps; ++rep )
            {
                for( Size_T i = 0; i < n; ++i )
                {
                    R_S[i] = std::invoke( f, A_S[i], B_S[i], C_S[i] );
                }
            }
        }
        toc("Computing f");
        ...
        tic("Computing g");
        {
            for( Size_T rep = 0; rep < reps; ++rep )
            {
                for( Size_T i = 0; i < n; ++i )
                {
                    R_T[i] = std::invoke( g, A_T[i], B_T[i], C_T[i] );
                }
            }
        }
        toc("Computing g");
