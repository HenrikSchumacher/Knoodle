...
template<bool mQ>
bool SubtreesCollideQ_Recursive2( const Int i )
{
    const bool interiorQ = InteriorNodeQ(i);
    ...
    if( interiorQ )
    {
        ...
        const NodeSplitFlagMatrix_T F = NodeSplitFlagMatrix2<mQ>(L,R);
        
        // TODO: We should prioritize SubtreesCollideQ_Recursive(i) if NodeBegin(i) is closer to a pivot than NodeEnd(i)-1.
        
        if( (F[0][0] && F[0][1]) && SubtreesCollideQ_Recursive2<mQ>(L) )
        {
            return true;
        }
        
        if( (F[1][0] && F[1][1]) && SubtreesCollideQ_Recursive2<mQ>(R) )
        {
            return true;
        }
        
        if( ( (F[0][0] && F[1][1]) || (F[0][1] && F[1][0]) ) && BallsOverlapQ(L,R) &&SubtreesCollideQ_Recursive2<mQ>(L,R) )
        {
            return true;
        }
    }
...
template<bool mQ>
bool SubtreesCollideQ_Recursive2( const Int i, const Int j )
{
    const bool i_interiorQ = InteriorNodeQ(i);
    const bool j_interiorQ = InteriorNodeQ(j);
    ...
    if( i_interiorQ && j_interiorQ )
    {
        ...
        const NodeSplitFlagMatrix_T F_i = NodeSplitFlagMatrix2<mQ>(c_i[0],c_i[1]);
        const NodeSplitFlagMatrix_T F_j = NodeSplitFlagMatrix2<mQ>(c_j[0],c_j[1]);
        ...
        if( subdivideQ[0][0] && SubtreesCollideQ_Recursive2<mQ>(c_i[0],c_j[0]) )
        {
            return true;
        }
        
        if( subdivideQ[1][1] && SubtreesCollideQ_Recursive2<mQ>(c_i[1],c_j[1]) )
        {
            return true;
        }
        
        if( subdivideQ[0][1] && SubtreesCollideQ_Recursive2<mQ>(c_i[0],c_j[1]) )
        {
            return true;
        }
        
        if( subdivideQ[1][0] && SubtreesCollideQ_Recursive2<mQ>(c_i[1],c_j[0]) )
        {
            return true;
        }
    }
    else if( !i_interiorQ && j_interiorQ )
    {
        ...
        const NodeSplitFlagVector_T f_i = NodeSplitFlagVector2<mQ>(j);
        const NodeSplitFlagMatrix_T F_j = NodeSplitFlagMatrix2<mQ>(c_j[0],c_j[1]);
        ...
        if( subdivideQ[0] && SubtreesCollideQ_Recursive2<mQ>(i,c_j[0]) )
        {
            return true;
        }
        
        if( subdivideQ[1] && SubtreesCollideQ_Recursive2<mQ>(i,c_j[1]) )
        {
            return true;
        }
    }
    else if( i_interiorQ && !j_interiorQ )
    {
        ...
        const NodeSplitFlagMatrix_T F_i = NodeSplitFlagMatrix2<mQ>(c_i[0],c_i[1]);
        const NodeSplitFlagVector_T f_j = NodeSplitFlagVector2<mQ>(j);
        ...
        if( subdivideQ[0] && SubtreesCollideQ_Recursive2<mQ>(c_i[0],j) )
        {
            return true;
        }
        
        if( subdivideQ[1] && SubtreesCollideQ_Recursive2<mQ>(c_i[1],j) )
        {
            return true;
        }
    }
