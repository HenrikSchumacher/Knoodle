...
template<Size_T t, bool appendQ = true>
void kv( std::string_view key, cref<Tiny::Vector<AmbDim,Real,Int>> value )
{
    log << ((appendQ ? ",\n" : "\n") + ct_tabs<t> + "\"") << key << "\" -> { " << ToMathematicaString(value[0]) << ", " << ToMathematicaString(value[1]) << ", " << ToMathematicaString(value[2]) <<" }";
}

template<Size_T t, bool appendQ = true>
void kv( std::string_view key, cref<char *> value )
{
    log << ((appendQ ? ",\n" : "\n") + ct_tabs<t> + "\"" + key + "\" -> \"") << value << "\"";
}

template<Size_T t, bool appendQ = true, typename T >
void kv( std::string_view key, cref<T> value )
{
    log << ((appendQ ? ",\n" : "\n") + ct_tabs<t> + "\"") << key << "\" -> " << ToString(value);
}

template<Size_T t, bool appendQ = true>
void kv( std::string_view key, cref<double> value )
{
    log << ((appendQ ? ",\n" : "\n") + ct_tabs<t> + "\"") << key << "\" -> " << ToMathematicaString(value);
}

template<Size_T t, bool appendQ = true>
void kv( std::string_view key, cref<float> value )
{
    log << ((appendQ ? ",\n" : "\n") + ct_tabs<t> + "\"") << key << "\" -> " << ToMathematicaString(value);
}
