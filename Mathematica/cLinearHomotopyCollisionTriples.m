...
Quiet[LibraryFunctionUnload[cLinearHomotopyCollisionTriples]];
ClearAll[cLinearHomotopyCollisionTriples];

cLinearHomotopyCollisionTriples::usage="cLinearHomotopyCollisionTriples[ t0, P0, t1, P1] computes the collision information for the linear interpolation between the cyclic polygon P0 and time t0 and the cyclic polygon P1 at time t1. Suppose that the linear homotopy is parameterized by the map f : [t0,t1] x [0,1] -> R^3. Then this routine returns the list of all triples {t,x,y} satisfying f[t,x] == f[t,y].";

cLinearHomotopyCollisionTriples := cLinearHomotopyCollisionTriples = Module[{lib, libname, file, d, code, name, t, $logFile},
	
	d = 3;

	name = "cLinearHomotopyCollisionTriples";
